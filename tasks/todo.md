...
- [x] Add: `storage.googleapis.com` (GCS), `eventarc.googleapis.com` (Eventarc), `dataplex.googleapis.com` (Dataplex for Governance).
- [x] Retain: `run.googleapis.com` (Cloud Run), `aiplatform.googleapis.com` (Vertex AI), `bigquery.googleapis.com` (BigQuery), etc.
- [x] Remove: Unused APIs from the previous architecture (e.g., `firestore.googleapis.com`, `routeoptimization.googleapis.com`, `maps-backend.googleapis.com`, `redis.googleapis.com`).
...
- [x] `roles/storage.objectViewer` or `roles/storage.objectAdmin` on the raw GCS bucket.
- [x] `roles/bigquery.dataEditor` and `roles/bigquery.jobUser` for executing dbt models.
- [x] `roles/run.invoker` for Cloud Run execution.
- [x] `roles/eventarc.eventReceiver` for Eventarc triggers.
- [x] `roles/aiplatform.user` for Vertex AI/Gemini integration.
...
- [x] Apply Terraform Configuration to `manish-sandpit`.
...
- [x] Publish code/infrastructure to GitHub.

# Phase 3: Transformation & Orchestration (Dataform + Cloud Run) - ToDo

## Task 8: Set up BigQuery Connection/Utility
- [x] Create a Python utility for "Antigravity" to query BigQuery easily.
